...
def eval_when_training(cfg, model, dataloader, prec_):
    ...
    with torch.no_grad():
        ...
        p_pos, p_angle, p_slot = dataset.eval_once(point_pred_list, point_gt_list, slot_pred_list, slot_gt_list)
        print(f'VOC AP:     pos:{p_pos}      angle:{p_angle}        slot:{p_slot}')
        prec_['pos'].append(p_pos)
        prec_['angle'].append(p_angle)
        prec_['slot'].append(p_slot)