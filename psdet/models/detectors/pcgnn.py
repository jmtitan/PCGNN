...
from .util.post_process import get_predpoints_points
...
class PCGNN(nn.modules.Module):
    ...
    def forward(self, data_dict):
        ...
        points_pred = torch.sigmoid(point_pred)
        angle_pred = torch.tanh(angle_pred)
        data_dict['points_pred'] = torch.cat((points_pred, angle_pred), dim=1)
    ...
    def get_targets(self, data_dict):
        ...
        for batch_idx, marks_gt in enumerate(marks_gt_batch):
            ...
            for marking_point in marks_gt[:n]:
                x, y = marking_point[:2]
                cos, sin, length = marking_point[2:5]
                ...
                targets[batch_idx, 4, row, col] = cos
                targets[batch_idx, 5, row, col] = sin
    ...
    def post_processing(self, data_dict):
        ...
        for b, marks in enumerate(points_pred):
            points_pred = get_predpoints_points(marks, self.cfg.point_thresh, self.cfg.boundary_thresh)  # nms
