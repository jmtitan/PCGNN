...
from .util.post_process import get_predpoints_dcl
...
class PCGNNDCL(nn.modules.Module):
    ...
    def __init__(self, cfg, inter_threshs):
        ...
        self.angle_loss_func = FocalLoss().cuda()
    ...
    def get_targets(self, data_dict):
        ...
        for batch_idx, marks_gt in enumerate(marks_gt_batch):
            ...
            for marking_point in marks_gt[:n]:
                ...
                targets[batch_idx, 5:, row, col] = dcl_label
                # indices = [batch_idx, row, col]
    ...
    def get_training_loss(self, data_dict):
        ...
        disp_dict = {}
        b, c, r = torch.nonzero(targets[:, 0, :, :], as_tuple=True) # 选取目标角点的特征图位置

        # point_loss
        loss_point = self.point_loss_func(points_pred[:, :5, :, :] * mask[:, :5, :, :], targets[:, :5, :, :] * mask[:, :5, :, :]) # 角点位置不能用indice， 因为需要回归各个网格点的置信度

        # angle loss
        angle_pred = points_pred[b, 5:, c, r]
        angle_gt = targets[b, 5:, c, r]

        # 正则项
        with torch.no_grad():
            decode_angle_labels = bin2dec(angle_gt, 8) * -1
            decode_angle_preds = bin2dec(angle_pred, 8) * -1
            diff_w = torch.abs(torch.sin(torch.deg2rad(decode_angle_labels - decode_angle_preds))).view(-1, 1)

        loss_angle = self.angle_loss_func(angle_pred, angle_gt.long(), diff_w)
    ...
    def post_processing(self, data_dict):
        ...
        for b, marks in enumerate(points_pred):
            points_pred = get_predpoints_dcl(marks, self.cfg.point_thresh, self.cfg.boundary_thresh)  # nms
